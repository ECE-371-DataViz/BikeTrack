...
def display_on_leds(colors, duration=None, brightness=0.1):
    ...
    leds = neopixel.NeoPixel(
        board.D18, NUM_LEDS, brightness=brightness, auto_write=False
    )
...
def main():
    parser = argparse.ArgumentParser(description="Display image colors on LED strip")
    parser.add_argument("--input", required=True, help="Input CSV file with LED colors")
    parser.add_argument(
        "--duration",
        type=float,
        default=None,
