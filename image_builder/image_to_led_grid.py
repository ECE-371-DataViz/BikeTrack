...
def lat_lon_to_image_coords(
    lat, lon, center_lat, center_lon, size, img_width, img_height
):
...
def generate_preview_map(results, center_lat, center_lon, size, output_path):
    ...
    m = folium.Map(
        location=[center_lat, center_lon], zoom_start=14, tiles="CartoDB dark_matter"
    )
    ...
    folium.PolyLine(
        bounds, color="yellow", weight=2, opacity=0.8, popup="Image Bounds"
    ).add_to(m)
...
def main():
    ...
    parser.add_argument(
        "--center-lat", type=float, required=True, help="Latitude of center point"
    )
    parser.add_argument(
        "--center-lon", type=float, required=True, help="Longitude of center point"
    )
    parser.add_argument(
        "--size",
        type=float,
    ...
    parser.add_argument(
        "--output",
        default="led_output.csv",
        help="Output CSV file path (default: led_output.csv)",
    )
    parser.add_argument(
        "--data-src",
        default="data_src.csv",
        help="Path to data_src.csv (default: data_src.csv)",
    )
    parser.add_argument(
        "--preview", action="store_true", help="Generate preview HTML map"
    )
    ...
    results, in_bounds = sample_image_at_leds(
        args.image, leds, args.center_lat, args.center_lon, args.size
    )
    ...
    if args.preview:
        ...
        generate_preview_map(
            results, args.center_lat, args.center_lon, args.size, preview_path
        )
