...
def fetch_station_info(url: str) -> List[Dict]:
    if requests is None:
        raise RuntimeError(
            "requests package is required to fetch GBFS data; install it with pip install requests"
        )
...
def map_labels_to_ids(
    labels_csv: str, url: str, fuzzy_threshold: float = 0.8
) -> Tuple[List[Tuple[str, str, str, float]], List[Tuple[str, str]]]:
    ...
    with open(labels_csv, newline="") as f:
        ...
        for row in reader:
            ...
            if normalized in name_map:
                mapped.append(
                    (
                        index,
                        name_map[normalized],
                        station_names[station_names.index(normalized)],
                        1.0,
                    )
                )
                continue
            ...
            close = get_close_matches(
                normalized, station_names, n=1, cutoff=fuzzy_threshold
            )
            if close:
                ...
                candidates = [
                    ...
                    if normalized in n
                    or any(token in n for token in normalized.split() if len(token) > 2)
                ]
                if candidates:
...
def cli():
    ...
    p.add_argument(
        "--in", dest="in_csv", required=True, help="Input LED CSV: index,label"
    )
    p.add_argument(
        "--out",
        dest="out_csv",
        required=True,
        help="Output CSV: index,station_id,matched_name,score",
    )
    p.add_argument(
        "--url", default=GBFS_URL_DEFAULT, help="GBFS station_information URL"
    )
    p.add_argument(
        "--fuzzy-threshold",
        type=float,
        default=0.86,
        help="Difflib cutoff threshold 0..1",
    )
    args = p.parse_args()
